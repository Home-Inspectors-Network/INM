...
export default function InspectorProfile() {
  ...
  const fetchReviews = async () => {
    try {
      const response = await fetch(`/api/inspectors/${id}/reviews`);
      if (response.ok) {
        const data = await response.json();
        setReviews(data);
      }
  ...
  if (!inspector) {
    return (
      <Layout title="Inspector Not Found">
        <div className="container py-12 text-center">
          <h1 className="text-2xl font-bold mb-4">Inspector Not Found</h1>
          <p className="text-secondary-600 mb-8">
            The inspector you're looking for doesn't exist or has been removed.
          </p>
  ...
  return (
    <Layout
      title={`${business_name} - Home Inspector in ${city}, ${state}`}
      description={`Professional home inspection services by ${business_name} in ${city}, ${state}. Licensed, insured, and experienced.`}
    >
              ...
              <div className="space-y-8">
                <div className="card p-6">
                  ...
                  <p className="text-secondary-600">
                    {company_description || (
                      `Professional home inspection services in ${city}, ${state}.
