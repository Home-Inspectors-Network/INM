...
/**
 * TOP 10 COMPREHENSIVE COLLECTOR - PHASE 2
 * 
...
class TermiteInspectorCollector {
  constructor() {
    this.stats = {
      total_processed: 0,
      total_collected: 0,
    ...
    this.allInspectors = [];
    this.logDir = path.join(__dirname, '../logs/termite-collection');
    fs.ensureDirSync(this.logDir);
  }

  async collectAllCities() {
    console.log('🐜 TOP 10 TERMITE/PEST INSPECTORS COLLECTION - PHASE 2');
    console.log('====================================================');
  ...
  async collectCityInspectors(city) {
    ...
    try {
      // Generate search queries for termite/pest inspectors
  ...
  generateSearchQueries(city) {
    return [
      `termite inspection ${city.name} ${city.state}`,
      `pest control inspector ${city.name} ${city.state}`,
      `WDO inspection ${city.name} ${city.state}`,
      `termite inspector near ${city.name}`,
      `"termite inspection company" ${city.name}`,
      `wood destroying organism inspector ${city.name}`,
      `pest inspection services ${city.name} ${city.state}`,
      `licensed termite inspector ${city.name}`,
      `termite and pest inspection ${city.shortName}`,
      `structural pest inspector ${city.name}`
    ];
  }
  ...
  isValidInspectorResult(result) {
    ...
    const excludePatterns = [
      ...
      'orkin.com/locations', 'terminix.com/locations' // Exclude location pages
    ];
    ...
    // Must have termite/pest inspection keywords
    const keywords = ['termite', 'pest', 'wdo', 'wood destroying', 'inspection'];
    const hasKeyword = keywords.some(keyword => 
      title.includes(keyword) || content.includes(keyword)
    );
    
    // Must specifically mention inspection, not just treatment
    const inspectionKeywords = ['inspect', 'inspection', 'inspector', 'assessment', 'evaluation'];
  ...
  extractBusinessName(title, url) {
    ...
    let name = title;
    const suffixes = [
      ' - Termite Inspector', ' - Pest Control', ' | Termite Inspection', ' - Yelp', ' - Google',
      ' Pest Control Services', ' - Pest Inspector', ' Termite & Pest', ' | Pest Inspection'
    ];
    ...
    name = name.replace(/^(Termite Inspector|Pest Inspector|Inspector):\s*/i, '');
    name = name.trim();
    
    // If name is too generic, try to extract from URL
    if (name.length < 3 || ['termite', 'pest', 'inspector'].includes(name.toLowerCase())) {
  ...
  extractServices(content) {
    ...
    const serviceKeywords = {
      'Termite Inspection': ['termite inspection', 'termite assessment', 'termite evaluation'],
      'WDO Inspection': ['wdo inspection', 'wood destroying organism', 'wood destroying insect'],
      'Pre-Purchase Pest Inspection': ['pre-purchase pest', 'buyer pest inspection', 'real estate pest'],
      'Pest Inspection': ['pest inspection', 'pest assessment', 'general pest inspection'],
      'Structural Pest Control': ['structural pest', 'structural damage inspection'],
      'Commercial Pest Inspection': ['commercial pest inspection', 'business pest inspection'],
      'Annual Termite Inspection': ['annual termite', 'yearly termite', 'termite warranty'],
      'Moisture Inspection': ['moisture inspection', 'moisture assessment', 'fungus inspection'],
      'Wood Rot Inspection': ['wood rot', 'dry rot inspection', 'fungal damage'],
      'Termite Treatment': ['termite treatment', 'termite control', 'termite remediation']
    };
    ...
    // Always include basic termite inspection if termite inspector
    if (services.length === 0) {
      services.push('Termite Inspection');
    }
  ...
  extractCertifications(content) {
    ...
    const certPatterns = {
      ...
      'NPMA': /\bNPMA\b/i,
      'TPCL': /\bTPCL\b/i,
      'ACE': /\b(?:ACE certified|Associate Certified Entomologist)\b/i,
      'BCE': /\b(?:BCE|Board Certified Entomologist)\b/i,
  ...
  extractDescription(content) {
    // Get first meaningful paragraph about termite/pest inspection
    const sentences = content.split(/[.!?]/).filter(s => s.length > 50 && s.length < 300);
    
    for (const sentence of sentences) {
      // Look for sentences mentioning termite or pest inspection
      const lowerSentence = sentence.toLowerCase();
      if ((lowerSentence.includes('termite') || lowerSentence.includes('pest')) && 
  ...
  extractLicense(content) {
    ...
    const patterns = [
      ...
      /lic\s*#?\s*:?\s*([A-Z0-9-]+)/i,
      /(?:TPCL|State)\s*#?\s*:?\s*([A-Z0-9-]+)/i,
      /pest\s*control\s*license\s*#?\s*:?\s*([A-Z0-9-]+)/i
    ];
  ...
  validateInspector(inspector) {
    ...
    const termiteKeywords = ['termite', 'pest', 'wdo', 'wood destroying'];
    const hasTermiteService = termiteKeywords.some(keyword => 
      name.includes(keyword) || services.includes(keyword) || description.includes(keyword)
    );
    
    return hasTermiteService;
  }
  ...
  async finalReport() {
    ...
    const reportPath = path.join(this.logDir, `termite-collection-report-${Date.now()}.json`);
    ...
    const { count } = await supabase
      ...
      .eq('inspector_type', 'termite');
    
    console.log(`\n💾 Database Status:`);
    console.log(`   Total Termite Inspectors: ${count}`);
...
if (require.main === module) {
  const collector = new TermiteInspectorCollector();
  
  collector.collectAllCities()
    .then(() => {
      console.log('\n✅ Termite inspector collection completed successfully!');
      process.exit(0);
    })
    .catch(error => {
...
module.exports = TermiteInspectorCollector;